...
with st.sidebar:
    ...
    category = st.selectbox("Categoria*", ["1a Nacional", "SF2 (Superlliga 2)"])
...
players_df = st.data_editor(
    ...
    num_rows="dynamic",
    columns={
        "Posició": st.column_config.SelectboxColumn(options=["Col·locador","Opost","Central","Receptor","Líbero"]),
        "Número dorsal": st.column_config.TextColumn(help="Ex.: 7"),
    },
...
staff_df = st.data_editor(
    ...
    num_rows="dynamic",
    columns={
        "Funció": st.column_config.SelectboxColumn(options=["Entrenador principal","Segon entrenador","Preparador físic","Fisioterapeuta","Delegat","Altres"]),
    },
    use_container_width=True,
    key="staff_editor"
)
    ...
    if export_xlsx:
        try:
            ...
            with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
                pd.DataFrame({"Equip":[team_name], "Sexe":[sex], "Categoria":[category]}).to_excel(writer, sheet_name="Equip", index=False)
                (players_df if not players_df.empty else pd.DataFrame(columns=["Equip","Sexe","Categoria", *player_cols])).to_excel(writer, sheet_name="Jugadors", index=False)
                (staff_df if not staff_df.empty else pd.DataFrame(columns=["Equip","Sexe","Categoria", *staff_cols])).to_excel(writer, sheet_name="Staff", index=False)
...
with col1:
    save_btn = st.button("💾 Desa equip", type="primary")
with col2:
    reset_btn = st.button("🧹 Reinicia formulari (nou equip)")
...
template_buf.seek(0)
st.download_button("⬇️ Descarrega plantilla Excel buida (3 pestanyes)", data=template_buf, file_name="plantilla_equip_voleibol.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
if save_btn:
    if not validate_team(team_name):
        ...
        try:
            saved_files = export(team_name, sex, category, players_df, staff_df, export_xlsx=export_xlsx, export_csv=export_csv)
            if saved_files:
                st.success("Dades desades correctament:")
                for f in saved_files:
                    st.write("• ", f)
                if send_email:
                    ok, msg = send_notification(saved_files, notif_to, smtp_server, smtp_port, smtp_user, smtp_pass, use_tls)
                    if ok:
                        st.info(msg)
                    else:
                        st.warning(msg)
